...
LM_AI 使用 WebView2 顯示桌面介面。目前的 NSIS 只安裝 LM_AI，不檢查、不包含或自動下載 WebView2，一般安裝完成也不自動啟動主程式。Windows 11 通常已包含 Runtime；若開啟 LM_AI 時提示缺少執行階段，請從公司另行提供的下載位置取得 `MicrosoftEdgeWebView2RuntimeInstallerX64.exe`，安裝後重新開啟 `LM_AI.exe`，不必重跑 Setup。Edge 瀏覽器與 WebView2 Runtime 是不同元件，不能只以安裝 Edge 判斷 Runtime 可用性。
