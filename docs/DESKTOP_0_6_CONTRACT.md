...
所有下列路徑都包含目前部署前綴 `/lm_server`。認證沿用 `Authorization: Bearer <personal_access_token>`，所有请求包含 `X-Client-Version: 0.6.0`。不要拿網站 Cookie 路由直接充當 desktop Token API。
...
規則只用於預檢，實際上傳及聊天仍由後端驗證。PDF／MSG 可能產生圖片：**若模型不能接收圖片，後端必須使用可支援的文字結果，或清楚拒絕，不可把圖片轉發给不支援的模型。** App 不知道伺服器轉檔結果的實際格式。
...
- **全站通知不因 App 在前景就自動已讀**，避免改變网站鈴鐺語意。明確點擊／全部已讀且 API 成功才更改。
...
1. 使用者可選 Outlook 多封郵件，或選預設帳號收件匣「所有／僅未讀 × 今天／三天內／本週」。三天含今天，本週從週一開始，使用本機日曆日期。
...
8. 全部附件 ready 後，App 自動送出最終整理，優先 background，否則 stream，沿用 0.5 持久任務／取消／恢復機制。這輪不再執行工具，避免無限自動調用。
