...
本次先交付 0.8.13 EXE、EXE 更新清單與原始碼。NSIS 保留 0.8.12，待使用者實測後再製作；不重製歷史離線 ZIP。發行驗證見 [0.8.13 驗收](VALIDATION_0_8_13.md)。
...
按更新後，先 GET 首頁取得 PHPSESSID，再 POST `uid`、`pwd`、`type=1` 至登入路由，改用登入回應的新 PHPSESSID 依序 GET 清單，最後 POST 登出。每次送出前等待 300ms，不自動重試／轉址；登入、API 或解析失敗及停止操作時，也會在已知 session 的情況下嘗試登出。登出失敗明確提示，不宣稱網站已結束 session；關機或強制終止程序則無法保證送出登出請求。流程依使用者提供的 PHP session 行為實作，採 [WinHTTP 手動 Cookie 處理](https://learn.microsoft.com/en-us/windows/win32/winhttp/cookie-handling-in-winhttp)。
...
取得成功後只在記憶體暫存，先顯示所有分類及機台，預設不勾選。可以勾選整類或個別機台，再按「匯入已選項目」；未勾選資料捨棄，亦可捨棄整份暫存。匯入採新增／更新，保留手動機台、既有 VNC 密碼及排序。新機台密碼預設為 `1234`，可於機台管理更改；空 IP 可匯入／編輯，但連線按鈕停用，原生層也拒絕空位址連線。

對應優先使用網站根目錄＋machine_id（保存為機台額外欄位 `_lm_sync_source`、`_lm_sync_id`）；未有識別碼的舊機台以相同分類與唯一名稱對應。再次同步更新名稱／IP，保留個人分類及密碼；重複名稱造成多筆匹配時提示先整理，不任意覆寫。網站未提供穩定 ID 時，改名可能成為新機台。
